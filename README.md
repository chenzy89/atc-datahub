...
- **指标**：把云图 PNG 上固定区域 `(550,280)-(780,380)` 裁出后重新压成 PNG 的字节数（KB）作为“云量代理”（云多→细节多→KB 大）；每小时对本小时全部云图（约每 6 分钟一张）取平均。
- **入库**：`cloud_cover(date, hour, avg_kb, count, level, created_at)`（`data/aftn.db`），只存统计值，**图片不入库**。`level` = 把 avg_kb 按阈值分 0–11 级。
- **数据源**：`/mnt/WXMap/<MMDD>/HHmm.PNG`（目录/文件名均为北京时，入库自动转 UTC）。挂载见上文 fstab（按需 automount）。
- **补扫提速（v2.3.13）**：补全/启动扫描时**先按名称换算目标小时，已入库的不再下载/裁剪**：
  - 某目录 24 个小时都已在库 → 连列目录都省掉（零共享盘访问，月视图补全从分钟级 → 毫秒级）
  - 部分缺失 → 只处理缺的小时；`当前小时/上一小时` 始终刷新（云图整点内仍会陆续到）
  - 前端月视图改为**先渲染已入库数据，再后台补全**，打开即出图
- **可选取样**：`config.json` 的 `cloud_cover.sample_per_hour`（默认 `0`=全部；设为如 `2` 则每小时只取前 2 张参与平均）——首次补全新历史时能再快 ~90%，代价是均值略粗。
